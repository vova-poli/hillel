...
# filter by criteria, add filtered data to filtered_car_data dictionary
for key, value in car_data.items():
  if value[1] >= search_criteria[0] and value[2] >= search_criteria[1] and value[4] <= search_criteria[2]:
    filtered_car_data[key] = value

# sort filtered_car_data by price, add data to sorted_by_price list
sorted_by_price = sorted(filtered_car_data.items(), key=lambda value: value[1][4])

# make slice of 5 cheapest cars
for key, value in sorted_by_price[:5]:
  print(f"{key}: {value}")